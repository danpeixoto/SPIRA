...
class FeatureExtractor():
    ...
    def __call__(self, x):
        ...
        for name, module in self.model._modules.items():
            if(name == "bn0"):
                # essa regra foi criada porque bn0 espera [batch, channel, heigth, width]
                x = module(x.unsqueeze(0).transpose(1, 3))
                # reverte para o normal
                x = x.transpose(1, 3)
            elif(name == "fc1"):
...
class ModelOutputs():
    ...
    def __call__(self, x):
        target_activations = []
        for name, module in self.model._modules.items():
            # print(x.shape, "Model outputs")
            # print(module == self.feature_module, name)
            # print(self.model, self.feature_module)
...
def show_cam_on_image(img, mask):
    ...
    # print(img.shape,len(np.expand_dims(mask, axis=2).shape))
...
class GradCam:
    ...
    def __call__(self, input_img, target_category=None):
        ...
        # print(output.shape)
        # one_hot = np.zeros((output.size()[0],1 ), dtype=np.float32)
        # one_hot[:][:] = 1
        ...
        cam = cv2.resize(cam, (64, 201))  # ALTERAR AQUI
        cam = cam - np.min(cam)
        cam = cam / np.max(cam)
        return cam, torch.round(output[0])

...
import torchaudio
from .generate_img import gerar_imagem
...
class Dataset(Dataset):
    ...
    def __init__(self, c, ap, train=True, max_seq_len=None, test=False):
        ...
        #rint(self.control_class)
        #rint(self.dataset_list[0])
    ...
    def __getitem__(self, idx):
        ...
        genero = self.dataset_list[idx][2]
        idade = self.dataset_list[idx][3]
        #print(self.dataset_list[idx][0])
        ...
        if self.c.dataset['split_wav_using_overlapping']:
            ...
            for end_slice in range(self.ap.sample_rate*self.c.dataset['window_len'], wav.shape[1], step):
                ...
                if (self.ap.feature == 'logmel'):
                    amp_to_db = torchaudio.transforms.AmplitudeToDB()
                    #rint(spec.shape)
                    spec = amp_to_db(spec)
                    #rint(spec.shape)
                    spec = gerar_imagem(wav,self.ap.sample_rate,spec,idade,genero) # aqui é onde gero a imagem com idade, f0 ,std f0, genero e spec
                    #rint(spec.shape)
                features.append(spec)
                targets.append(torch.FloatTensor([class_name]))
                start_slice += step
            if len(features) == 1:
                spec = self.ap.get_feature_from_audio(wav[:,:self.ap.sample_rate*self.c.dataset['window_len']]).transpose(1,2)
                if(self.ap.feature == 'logmel'):
                    amp_to_db = torchaudio.transforms.AmplitudeToDB()
                    #print(spec.shape)
                    spec = amp_to_db(spec)
        ...
        #print(torch.max(feature),torch.min(feature), True in torch.isnan(feature))
        if(True in torch.isnan(feature)):
            print(self.dataset_list[idx])
        #print(feature.shape)

...
class SpiraConvV2(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.padding_with_max_lenght = self.config.dataset[
            'padding_with_max_lenght'] or self.config.dataset['split_wav_using_overlapping']
        self.max_seq_len = self.config.dataset['max_seq_len']
        print("Model Inicialized With Feature %s " %
              self.config.audio['feature'])
        if self.config.audio['feature'] == 'spectrogram':
            self.num_feature = self.config.audio['num_freq']
        elif self.config.audio['feature'] == 'melspectrogram' or self.config.audio['feature'] == 'logmel':
            ...
            raise ValueError('Feature %s Dont is supported' %
                             self.config.audio['feature'])
        convs = [
            # cnn1
            nn.Conv2d(1, 32, kernel_size=(7, 1), dilation=(2, 1)),
            nn.GroupNorm(16, 32), Mish(), nn.MaxPool2d(
                kernel_size=(2, 1)), nn.Dropout(p=0.7),

            # cnn2
            nn.Conv2d(32, 16, kernel_size=(5, 1), dilation=(2, 1)),
            nn.GroupNorm(8, 16), Mish(),  nn.MaxPool2d(
                kernel_size=(2, 1)), nn.Dropout(p=0.7),

            # cnn3
            nn.Conv2d(16, 8, kernel_size=(3, 1), dilation=(2, 1)),
            nn.GroupNorm(4, 8), Mish(),  nn.MaxPool2d(
                kernel_size=(2, 1)), nn.Dropout(p=0.7),
            # cnn4
        ...
        if self.padding_with_max_lenght:
            ...
            # get zeros input
            # rint(self.num_feature)
            # 120 é o valor do total de features do experimento atual, o padrao e self.num_feature
            inp = torch.zeros(1, 1, self.max_seq_len, 120)
            # rint(self.num_feature)
            # rint(self.max_seq_len)
            # get out shape
            toy_activation_shape = self.conv(inp).shape
            # set fully connected input dim
            fc1_input_dim = toy_activation_shape[1] * \
                toy_activation_shape[2]*toy_activation_shape[3]
            # rint(fc1_input_dim)
            ...
            inp = torch.zeros(1, 1, 500, 120)
            # get out shape
            self.fc1 = nn.Linear(
                4*self.conv(inp).shape[-1], self.config.model['fc1_dim'])
        self.mish = Mish()
        self.fc2 = nn.Linear(
            self.config.model['fc1_dim'], self.config.model['fc2_dim'])
    ...
    def forward(self, x):
        ...
        # x: [B, 1, T, num_feature]
        x = self.conv(x)
        #rint('depois da conv',x.shape)
        # x: [B, n_filters, T, num_feature]
        x = x.transpose(1, 2).contiguous()
        # x: [B, T, n_filters, num_feature]
        #rint('depois do transpose', x.shape)
        if self.padding_with_max_lenght:
            # x: [B, T*n_filters*num_feature]
            x = x.view(x.size(0), -1)
        else:
...
class SpiraConvV1(nn.Module):
    def __init__(self, config):
        ...
        convs = [
            # cnn1
            nn.Conv2d(1, 32, kernel_size=(7, 1), dilation=(2, 1)),
            nn.BatchNorm2d(32), Mish(), nn.MaxPool2d(
                kernel_size=(2, 1)), nn.Dropout(p=0.7),

            # cnn2
            nn.Conv2d(32, 16, kernel_size=(5, 1), dilation=(2, 1)),
            nn.BatchNorm2d(16), Mish(),  nn.MaxPool2d(
                kernel_size=(2, 1)), nn.Dropout(p=0.7),

            # cnn3
            nn.Conv2d(16, 8, kernel_size=(3, 1), dilation=(2, 1)),
            nn.BatchNorm2d(8), Mish(),  nn.MaxPool2d(
                kernel_size=(2, 1)), nn.Dropout(p=0.7),
            # cnn4
            nn.Conv2d(8, 4, kernel_size=(2, 1), dilation=(1, 1)),

...
def test(criterion, ap, model, c, testloader, step,  cuda, confusion_matrix=False, debug=False):
    ...
    with torch.no_grad():
        for feature, target, slices, targets_org, name in testloader:
            ...
            if cuda:
                ...
                target = target.cuda()
            print(type(feature), feature.shape, target.shape)
            break
    ...
    print("Test\n ", "Acurracy: ", mean_acc, "Acurracy Control: ", acc_control,
          "Acurracy Patient: ", acc_patient, "Acurracy Balanced", acc_balanced)
    print("Loss:", mean_loss, "Loss Control:", loss_control,
          "Loss Patient:", loss_patient, "Loss balanced: ", loss_balanced)
    print("F1:", f1, "UAR:", uar)
    return mean_acc
...
def run_test(args, checkpoint_path, testloader, c, model_name, ap, cuda=True, debug=False):
    ...
    if checkpoint_path is not None:
        ...
        try:
            ...
            raise ValueError(
                "You need pass a valid checkpoint, may be you need check your config.json because de the of this checkpoint cause the error: " + e)
    ...
    model.train(False)
    test_acc = test(criterion, ap, model, c, testloader, step,
                    cuda=cuda, confusion_matrix=True, debug=debug)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--spira_noise', dest='spira_noise',
                        action='store_true')
    parser.add_argument('--no_spira_noise',
                        dest='spira_noise', action='store_false')
    parser.set_defaults(spira_noise=True)
    ...
    print("Insert spira noise ?", c.data_aumentation['insert_spira_noise'])
    ...
    run_test(args, args.checkpoint_path, test_dataloader, c,
             c.model_name, ap, cuda=False, debug=args.debug)

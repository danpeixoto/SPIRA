...
#                 final_noise_audio = numpy.sqrt(
#                     10 ** ((clean_db - noise_db - noise_snr) / 10)) * noise_wav
#             else:
#                 final_noise_audio += (numpy.sqrt(10 **
#                                       ((clean_db - noise_db - noise_snr) / 10)) * noise_wav)
#         if final_noise_audio is None:
...
#         accepted_temporal_control = [
#             'overlapping', 'padding', 'avgpool', 'speech_t', 'one_window']
#         assert (self.c.dataset['temporal_control']
#                 in accepted_temporal_control), "You cannot use the padding_with_max_length option in conjunction with the split_wav_using_overlapping option, disable one of them !!"

#         self.control_class = c.dataset['control_class']
#         self.patient_class = c.dataset['patient_class']

#         # read csvs
#         self.dataset_list = pd.read_csv(self.dataset_csv, sep=',').replace({'?': -1}).replace(
#             {'negative': self.control_class}, regex=True).replace({'positive': self.patient_class}, regex=True).values
...
#                 self.c.data_aumentation['musan_path'], noisetypes=self.c.data_aumentation['noisetypes'])
#         else:
...
#                 raise RuntimeError(
#                     "ERROR: when  test_insert_noise is True, num_test_additive_noise or  num_test_specaug need to be > 0")
#             if self.c.dataset['temporal_control'] == 'overlapping' or self.c.dataset['temporal_control'] == 'speech_t':
#                 raise RuntimeError(
#                     "ERROR: Noise insertion in  'temporal_control' overlapping and  speech_t is not supported !! You need implement it !!")
#         else:
...
#                     wav[:, start_slice:end_slice]).transpose(1, 2)
...
#                 zeros = torch.zeros(1, self.max_seq_len -
#                                     feature.size(1), feature.size(2))
...
#             feature = self.ap.get_feature_from_audio(
#                 wav[:, idx:idx+step]).transpose(1, 2)
#             target = torch.FloatTensor([class_name])
#         else:
...
#                 if self.c.dataset['temporal_control'] == 'padding':
#                     # padding for max sequence
#                     zeros = torch.zeros(feature.size(
#                         0), self.max_seq_len - feature.size(1), feature.size(2))
#                     # append zeros before features
#                     feature = torch.cat([feature, zeros], 1)
#                 # print(feature.shape)
#                 features.append(feature)
...
#                 targets_aug = torch.FloatTensor(
#                     [class_name]).repeat(self.num_test_specaug, 1)
#                 features_aug = features_aug.repeat(self.num_test_specaug, 1, 1)
#                 # apply spec augmentation in the features
#                 features_aug = self.spec_augmenter(
#                     features_aug.unsqueeze(1), test=True).squeeze(1)
#                 if self.c.dataset['temporal_control'] == 'padding':
#                     # padding for max sequence
#                     zeros = torch.zeros(features_aug.size(
#                         0), self.max_seq_len - features_aug.size(1), features_aug.size(2))
#                     # append zeros before features
...
#         class_count = np.array([len(np.where(classes_list == c)[0])
...
#         class_dataset_samples_weight = torch.from_numpy(
#             samples_weight).double()
#         # create sampler
#         sampler = torch.utils.data.sampler.WeightedRandomSampler(
#             class_dataset_samples_weight, len(class_dataset_samples_weight), replacement=True)
#     else:
#         sampler = None
#         shuffle = True
...
#             features, batch_first=True, padding_value=0).squeeze(2)
#         targets = torch.cat(targets, dim=0)
#     else:
...
class SpiraNoise(object):
    def __init__(self, ap, noise_min_amp, noise_max_amp, noise_path, num_noise_control, num_noise_patient, control_class, patient_class, seed, is_test=False):
        self.ap = ap  # audioprocessor
        ...
        self.num_noise_patient = num_noise_patient
        self.num_noise_control = num_noise_control
        self.noise_list = []
        augment_files = glob.glob(os.path.join(
            noise_path, '*.wav'))  # DANIEL: alterei o path
        for file in augment_files:
            self.noise_list.append(file)
        print("Patient class : ", self.patient_class)
        print("Control class : ", self.control_class)
        print(self.noise_list)

    def insert_noise_into_the_audio(self, audio, audio_index, class_name):
        ...
        if self.is_test:
            ...
            random.seed(self.seed*audio_index)
            torch.manual_seed(self.seed*audio_index)
            torch.cuda.manual_seed(self.seed*audio_index)
            np.random.seed(self.seed*audio_index)

        num_noise = 0
        if self.control_class == class_name:
            num_noise = self.num_noise_control
        elif self.patient_class == class_name:
            num_noise = self.num_noise_patient

        for _ in range(num_noise):
            noise_wav = self.ap.load_wav(
                self.noise_list[random.randint(0, len(self.noise_list))-1])
            noise_wav_length = noise_wav.shape[1]
            audio_len = audio.shape[1]
            noise_start_slice = random.randint(
                0, noise_wav_length-(audio_len+1))

            noise_wav = noise_wav[:,
                                  noise_start_slice:noise_start_slice+audio_len]

            max_amp = random.uniform(self.noise_min_amp, self.noise_max_amp)

            reduct_factor = max_amp/float(noise_wav.max().numpy())

            noise_wav = noise_wav * reduct_factor

            audio = audio + noise_wav
...
class AugmentWAV(object):
    ...
    def __init__(self, musan_path, noisetypes=['noise', 'speech', 'music']):
        ...
        self.noiselist = {}
        augment_files = glob.glob(os.path.join(
            musan_path, '*.wav'))  # DANIEL: alterei o path
        for file in augment_files:
            noise_key = file.split("/")[-3]

            if noise_key == "Ruidos":
                noise_key = "noise"

            if not noise_key in self.noiselist:
                self.noiselist[noise_key] = []
            self.noiselist[noise_key].append(file)
...
class Dataset(Dataset):
    ...
    def __init__(self, c, ap, train=True, max_seq_len=None, test=False, test_insert_noise=False, num_test_additive_noise=0, num_test_specaug=0):
        ...
        if self.c.data_aumentation['insert_spira_noise']:
            self.spira_noise = SpiraNoise(self.ap, self.c.data_aumentation['noise_min_amp'], self.c.data_aumentation['noise_max_amp'], self.c.data_aumentation['spira_noise_path'],
                                          self.c.data_aumentation['num_noise_control'], self.c.data_aumentation['num_noise_patient'], self.control_class, self.patient_class, self.c.train_config['seed'], self.test)
    ...
    def __getitem__(self, idx):
        ...
        if self.c.data_aumentation['insert_spira_noise']:
            wav = self.spira_noise.insert_noise_into_the_audio(
                wav, idx, class_name)
        ...
        if self.c.dataset['temporal_control'] == 'overlapping' or self.c.dataset['temporal_control'] == 'speech_t':
            ...
            for end_slice in np.arange(self.ap.sample_rate*self.c.dataset['window_len'], wav.shape[1], step):
                ...
                _, phase = librosa.magphase(spec_to_get_phase)
                phase = torch.from_numpy(phase).unsqueeze(0)
                phases.append(phase)

                spec = self.ap.get_feature_from_audio(
                    wav[:, start_slice:end_slice]).transpose(1, 2)
                # print(torch.max(spec))
                # print(np.array(spec).shape)
                features.append(spec)
                targets.append(torch.FloatTensor([class_name]))
                start_slice += step
        ...
        # print("After Spec:", feature.shape, target.shape)
        if self.test:
            return feature, target, wavfile_name, phases
        return feature, target
...
def teste_collate_fn(batch):
    ...
    names = []
    phases = []
    for feature, target, file_name, phase in batch:
        features.append(feature)
        targets.append(target)
        names.append(file_name)
        phases.append(phase)
    ...
    #features = stack(features, dim=0)
    #print(features.shape, targets.shape)
    return features, targets, slices, targets_org, names, phases

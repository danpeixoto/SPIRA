		...
		if self.c.dataset['split_wav_using_overlapping']:
			...
			for end_slice in range(self.ap.sample_rate*self.c.dataset['window_len'], wav.shape[1], step):
				...
				if(self.ap.feature == 'spectrogram-melspec'):
					spec_to_get_phase = librosa.stft(wav[:, start_slice:end_slice].numpy()[0], n_fft= self.ap.n_fft,hop_length=self.ap.hop_length, win_length=self.ap.win_length)
					mel_spec = self.ap.get_feature_from_audio(wav[:, start_slice:end_slice])
			...
			if len(features) == 1:
				if(self.ap.feature == 'spectrogram-melspec'):
					spec_to_get_phase = librosa.stft(wav[:, :self.ap.sample_rate*self.c.dataset['window_len']].numpy()[0], n_fft= self.ap.n_fft,hop_length=self.ap.hop_length, win_length=self.ap.win_length)
					mel_spec = self.ap.get_feature_from_audio(wav[:, :self.ap.sample_rate*self.c.dataset['window_len']])
				else:
					mel_spec = self.ap.get_feature_from_audio(wav[:, :self.ap.sample_rate*self.c.dataset['window_len']])
			...
			if(self.ap.feature == 'spectrogram-melspec'):
				audio_class = torchaudio.transforms.Spectrogram(n_fft=self.ap.n_fft, win_length=self.ap.win_length, hop_length=self.ap.hop_length)

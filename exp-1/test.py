...
def use_grad_cam(model,testloader):
	...
	img = images[21].unsqueeze(1).transpose(0,1).contiguous().transpose(0,2).contiguous()
	# print("greyscale_cam")
...
def run_test(args, checkpoint_path, testloader, c, model_name, ap, cuda=True):
	...
	if checkpoint_path is not None:
		...
		try:
			...
			raise ValueError("You need pass a valid checkpoint, may be you need check your config.json because de the of this checkpoint cause the error: {} ".format(e))       
